...
locals {
  vgw_name = "${var.name == "" ? "VGW" : "VGW_${var.name}"}"
  cgw_name = "${var.name == "" ? "CGW" : "CGW_${var.name}"}"
  vpn_name = "${var.name == "" ? "VPN" : "VPN_${var.name}"}"
}
...
resource "aws_vpn_gateway" "this" {
  ...
  tags            = "${merge(var.tags, map("Name", local.vgw_name))}"
}
...
resource "aws_customer_gateway" "this" {
  ...
  type       = "ipsec.1"
  tags       = "${merge(var.tags, map("Name", "${local.cgw_name}_${count.index}"))}"
}
...
resource "aws_vpn_connection" "this" {
  ...
  customer_gateway_id = "${aws_customer_gateway.this.*.id[count.index]}"
  type                = "ipsec.1"
  static_routes_only  = "${var.static_routes_only}"
  tags                = "${merge(var.tags, map("Name", "${local.vpn_name}_${count.index}"))}"
}
